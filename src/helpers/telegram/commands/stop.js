	...
	controller.query.countQuery('id', 'humans', 'id', target.id)
		.then((isregistered) => {
			...
			if (isregistered) {
				...
				controller.query.updateQuery('humans', 'enabled', false, 'id', target.id).catch((O_o) => {})
				ctx.reply('✅').catch((O_o) => {
					controller.log.error(O_o.message)
				})
		...
		.catch((err) => {
			controller.log.error(`Telegram commando /stop errored with: ${err.message} (command was "${command.text}")`)
		})
}